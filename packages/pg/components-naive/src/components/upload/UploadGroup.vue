...
const modelValueDataComputed = computed(() => modelValueData.value);
// console.log('modelValue222222', props.value);
watch(
  () => props.modelValue,
  (newValue, oldValue) => {
    // console.log('modelValue', props.modelValue);
    // console.log('modelValue', newValue);
    // console.log('modelValue.oldValue', oldValue);
...
function modalUploadOk(val) {
  ...
  if (!modelValueData.value[activatedUpload.value]) {
    modelValueData.value[activatedUpload.value] = [];
  }
  if (val && val.data) {
    const sum =
      val.data.length + modelValueData.value[activatedUpload.value].length;
    if (
      uploadSetting.value &&
    ...
    val.data.forEach((item) => {
      ...
      modelValueData.value[activatedUpload.value].push(item);
    });
  }
  emit('ok', modelValueData.value);
}
...
const delUploadItem = (item, index, key) => {
  if (modelValueData.value[key] && modelValueData.value[key].length > 0) {
    modelValueData.value[key].splice(index, 1);
  }
};
/**
 * 改名
...
const changeFileName = (item, index, key) => {
  modelValueData.value[key][index].edit = !(
    modelValueData.value[key] && modelValueData.value[key][index]?.edit
  );
};
/**
 * 保存修改
        ...
        <n-image-group>
          ...
          >
            <draggable
              ...
              :list="modelValueDataComputed[group.key]"
              :move="checkMove"
