...
const [FormDrawerData, formDrawerApiData] = useVbenDrawer({
  connectedComponent: DataList,
});

function handleData(row) {
  formDrawerApiData.setData({
    // 表单值
    values: row,
    isUpdate: true,
  });
  formDrawerApiData.open();
}
</script>
  ...
  <div class="grid2 h-full p-2">
    <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvent">
      <template #operate="{ row }">
        ...
        />
        <vxe-button mode="text" title="字典配置" @click="handleData(row)">字典配置</vxe-button>
