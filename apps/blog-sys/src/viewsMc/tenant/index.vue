...
import { type RowVO, SexOptionsFormatter } from '@pg/types';
import {
...
const [DrawerAccount, DrawerApiAccount] = useVbenModal({
 ...
 * @param row
 */
const rowEventFounder = (row: RowVO) => {
 ...
 * @param row
 */
const rowEventFounderEdit = (row: RowVO) => {
  DrawerApiAccount.setData({
...
function rowEventGoTenantAccount(row) {
    ...
    <vxe-grid ref="xGrid" v-bind="gridOptions" v-on="gridEvent">
      <template #founderAll="{ row }">
        <div v-if="row.founder">
          <div>
            {{ row.founderName }} ( <span @click="rowEventGoTenantAccount(row)" class="text-blue-500 cursor-pointer">{{ row.founder }}</span> )
          </div>
        </div>
      </template>
...
<!--        <vxe-button icon="vxe-icon-ellipsis-v" size="small" status="primary" transfer type="text">-->
<!--          <template #dropdowns>-->
<!--            <vxe-button-->
<!--              v-if="row.founder <= 0"-->
<!--              content="创始人添加"-->
