...
import { typesGoodsFormatter, typeSpecificationsFormatter } from '@pg/types';
...
function selectTag(row) {
  if (tagsSelect.value.indexOf(row) === -1) {
    tagsSelect.value.push(row);
  }
}
function selectTagDel(row) {
  const index = tagsSelect.value.indexOf(row);
      ...
      <NLayout class="w-[calc(100%-160px)]">
        <NLayoutContent>
          <n-page-header
            v-if="tagsSelect"
            title="选中的"
            class="mb-2 border-b-slate-300 border-b border-b-solid pb-2"
          >
          ...
          <n-page-header
            v-if="getTitle"
            :title="getTitle"
            class="mb-2 border-b-slate-300 border-b border-b-solid"
          >
            <template #back></template>
          </n-page-header>
